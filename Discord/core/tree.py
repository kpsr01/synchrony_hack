...
from .utils.chat_formating import (
    humanize_list,
    inline,
)
...
class MeTree(CommandTree):
    ...
    async def sync(self, *args, guild: Optional[Snowflake] = None, **kwargs) -> List[AppCommand]:
        ...
        if guild:
            return commands
        async with (
            self.client._config.all() as cfg
        ):  # Assuming client has _config attribute, you might need to adjust this
            for command in commands:
                if command.type is discord.AppCommandType.chat_input:
                    cfg["enabled_slash_commands"][
                        command.name
                    ] = command.id  # Assuming config structure
    ...
    async def me_check_enabled(self) -> None:
        ...
        """
        enabled_commands = (
            await self.client.list_enabled_app_commands()
        )  # Assuming this method exists in your bot class
    ...
    async def on_error(
        ...
        **kwargs,
        ...
        if isinstance(error, CommandNotFound):
            await self._send_from_interaction(interaction, ("Command not found."))
            log.warning(
            ...
            interaction.client._last_exception = exception_log  # Assuming you want to keep this

            message = (
                await interaction.client._config.invoke_error_msg()
            )  # Assuming client has _config and invoke_error_msg
            if not message:
                if (
                    interaction.user.id in interaction.client.owner_ids
                ):  # Assuming client has owner_ids
                    message = inline(
                        ("Error in command '{command}'. Check your console or logs for details.")
                    )
                else:
                    message = inline(("Error in command '{command}'."))
            ...
            if len(error.missing_permissions) == 1:
                msg = ("I require the {permission} permission to execute that command.").format(
                    permission=formatted
                )
            else:
                msg = ("I require {permission_list} permissions to execute that command.").format(
            ...
            await self._send_from_interaction(
                interaction, ("That command is not available in DMs.")
            )
        elif isinstance(error, CommandOnCooldown):
            relative_time = discord.utils.format_dt(
                datetime.now(timezone.utc) + timedelta(seconds=error.retry_after), "R"
            )
            msg = ("This command is on cooldown. Try again {relative_time}.").format(
            ...
            await self._send_from_interaction(
                interaction, ("You are not permitted to use this command.")
            )
        else:
        ...
        if interaction.guild:
            if not (await self.client.ignored_channel_or_guild(interaction)):
                await self._send_interaction_check_failure(
                    interaction, ("This channel or server is ignored.")
                )
                return False
